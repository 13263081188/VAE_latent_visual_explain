...
class _GradCAM(_CAM):
    ...
    def _backprop(self, scores: Tensor, latent_pos: int) -> None:
        ...
        print("latent_pos", latent_pos, "_____________________")
        # print("scores_shape", scores.shape)
        # print(len(scores))
        # print(scores[0].size())
        # print(scores[1].size())
        # print(scores[2].size())
        loss = scores[:, latent_pos].sum()
        self.model.zero_grad()
        loss.backward(retain_graph=True)

    def _get_weights(self, latent_pos, scores):
...
class GradCAM(_GradCAM):
    ...
    Example::
        ...
        >>> cam(latent_pos=100, scores=scores)
    ...
    def _get_weights(self, latent_pos: int, scores: Tensor) -> Tensor:  # type: ignore[override]
        """Computes the weight coefficients of the hooked activation maps"""

        self.hook_g: Tensor
        # Backpropagate
        self._backprop(scores, latent_pos)
...
class SmoothGradCAMpp(_GradCAM):
    ...
    def _get_weights(self, latent_pos: int, scores: Optional[Tensor] = None) -> Tensor:
        ...
        for _idx in range(self.num_samples):
            ...
            out = self.model.reparameterize(out[1], out[2])
            self.model.zero_grad()
            self._backprop(out, latent_pos)
